...
class MessageLog {
  ...
  async consolidate(chatId) {
    ...
    try {
      ...
      // Ask Haiku to extract memories
      const response = await this.client.messages.create({
        model: 'claude-haiku-4-20250514',
        max_tokens: 500,
        system: `Extract important information from this conversation that's worth remembering long-term. Return a JSON array of memories to store, or an empty array if nothing is notable.

Each memory: {"content": "concise fact/decision/preference", "category": "fact|preference|decision|lesson|person|project|event|conversation|resource|pattern|context"}

Skip: greetings, small talk, routine acknowledgments. Keep: facts, decisions, preferences, people, projects, events, lessons learned.`,
